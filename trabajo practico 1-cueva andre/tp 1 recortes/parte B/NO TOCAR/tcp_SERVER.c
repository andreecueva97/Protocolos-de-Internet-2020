...
//        .estructura vacia (struct sockaddr *)&their_addr  ->donde se va a cargar la info. del CLIENTE
  ...
  for(i=0;message[i]!='\0';i++); //opcional sino usar strlen(message);//_________peilou,  lo q hace el sistema con la conexion que acaba de entrar__________________________  
  printf("\nbytes Enviados %d\n\n",i);
 
   if (send(new_fd,message,i, 0) == -1) perror("send");
   ...
   buf[numbytes] = '\0';//le agrega el \0 para poder imprimirlo //--luego imprime lo recibido----

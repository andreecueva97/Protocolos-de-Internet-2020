...
int main(int argc, char *argv[])
{
	...
	FD_ZERO(&master);FD_ZERO(&exception_fds);
	...
	{
		read_fds = master;
        write_fds=master; exception_fds=master;
		if(select(fdmax+1, &read_fds, &write_fds, &exception_fds, NULL) == -1)//-----------------select
		{
		...
		for(i = 0; i <= fdmax; i++)//recorro todos los sockets
		{
            ...
            {
                   //me esta tomando el conjunto de write y exception
